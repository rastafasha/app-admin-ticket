...
export class SubscriptoresComponent implements OnChanges {
  ...
  getEventos(): void {
    ...
    this.isLoading = true;
    this.eventoService.getClientsbyEvent(this.eventoSeleccionado.id).subscribe(
      (res: any) => {
        ...
        const ticketCountObservables = this.clients.map(client =>
          this.paymentService.getPaymentByEventbyClientId(this.eventoSeleccionado.id, client.id)
        );
  ...
  guardarAsistencia(client, event: any) {
    ...
    this.eventoService.updateAsistencia(this.eventoSeleccionado.id, client_id, asistencia).subscribe(
      (res: any) => {
  ...
  enviarCertificado(client) {
    console.log('enviando correo ', this.eventoSeleccionado.id, 'del cliente', client);
    this.eventoService.enviarCertificado(this.eventoSeleccionado.id, client.id).subscribe((resp: any) => {
